...
def test_practice():
    ...
    search_city = driver.find_element(By.XPATH, '//input[@class="form-control"]').send_keys("India")
    time.sleep(5)

    list_of_states = driver.find_elements(By.XPATH, '//table[@id="example"]/tbody/tr/td[2]')
    print("State", " | ", "ACQ", " | ",  "Rank")

    for state in list_of_states:
        if "India" in state.text:
            s1 = driver.find_element(locate_with(By.TAG_NAME, "p").to_left_of(state)).text
            s2 = driver.find_element(locate_with(By.TAG_NAME, "p").to_right_of(state)).text
